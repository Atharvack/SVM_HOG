...
def letter_seg(lines_img, x_lines, i):
    ...
    for e in range(len(letter)):
        if letter[e][0] < x_linescopy[0]:
            ...
            letter_img.append(255 - letter_img_tmp)
        else:
            x_linescopy.pop(0)
            word += 1
            letter_index = 1
...
def image_segmentation(filepath):
    ...
    print("\n Resizing Image........")
    width = 1320
    height = int(width * src_img.shape[0] / src_img.shape[1])
    ...
    all_segmented_letters = []
    for i in range(len(lines)):
        all_segmented_letters.extend(letter_seg(lines_img, x_lines, i))
    
    return all_segmented_letters

def plot_segmented_letters(segmented_letters, max_letters=100):
    import matplotlib.pyplot as plt
    num_letters = min(len(segmented_letters), max_letters)
    rows = int(np.ceil(np.sqrt(num_letters)))
    cols = int(np.ceil(num_letters / rows))
    
    fig, axes = plt.subplots(rows, cols, figsize=(15, 15))
    axes = axes.flatten()
    
    for i in range(num_letters):
        axes[i].imshow(segmented_letters[i], cmap='gray')
        axes[i].axis('off')
    
    for i in range(num_letters, len(axes)):
        axes[i].axis('off')
...
# # Usage
# segmented_letters = image_segmentation('WhatsApp Image 2024-12-09 at 7.20.46 PM.jpeg')
# plot_segmented_letters(segmented_letters)